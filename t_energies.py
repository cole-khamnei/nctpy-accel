...
def get_cti_block(A_norms, x0s, xfs, T=1, rho=1, dt = 0.001, device="cpu", ret_mem=False, **extras):
    ...
    A_norms = torch.tensor(A_norms.astype("float32"), **devp)
    ...
    n_batch, n_nodes = A_norms.shape[:2]
    n_integrate_steps = int(np.round(T / dt)) + 1

    I_b = torch.eye(n_nodes, **devp).repeat(n_batch, 1, 1)
    S_b, B_b, B_b_T = I_b, I_b, I_b.transpose(1, 2)

    T, rho, dt = torch.tensor([T, rho, dt], **devp)

    M = torch.cat([torch.cat([A_norms, (-B_b @ B_b_T) / (2 * rho)], dim=2),
                   torch.cat([- 2 * S_b, -A_norms.transpose(1, 2)], dim=2)],
                   dim=1)
    c = torch.cat([torch.zeros(xrs_b.shape, **devp), 2 * S_b @ xrs_b], dim=1)
    # Ignore MPS fallback warning, still faster on MPS than CPU
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        c = torch.linalg.solve(M, c)
    ...
    dd = xfs_b - (E11 @ x0s_b)  - (torch.cat([E11 - I_b, E12], dim=2) @ c)
    # Ignore MPS fallback warning, still faster on MPS than CPU
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        l0 = torch.linalg.solve(E12, dd)

    big_I = torch.eye(2 * n_nodes, **devp).repeat(n_batch, 1, 1)
    Ad = torch.linalg.matrix_exp(M * dt)
    Bd = (Ad - big_I) @ c
...
def get_cti_batch(A_norm, x0s, xfs, T=1, dt=0.001, rho=1, device=None):
    """ """
    ...
    A_block = np.tile(A_norm, (n_batch, 1, 1))
    ...
    for i in tqdm(range(n_blocks), leave=False, desc="Single subject trajectory"):
        sl = slice(n_batch * i, n_batch * (i + 1))
        block_results = get_cti_block(A_block, x0s[sl], xfs[sl],
